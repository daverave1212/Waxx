...
'''
Everything in parenthesis becomes EXPRESSION (recursively)
'''
...
''' Everything inside an if _____ : becomes an EXPRESSION'''
...
'''
We look inside each line, then inside each EXPRESSION in the line for structures like:
    if _ _ _ :
If we have:
    if EXPR :
We keep it that way
'''
def expressizeLineByConditionKeyword2(partList):
    newPartList = []
    for i, part in enumerate(partLine):
        if part.type == 'EXPRESSION':
            thisParsedPart = Part(
                content = expressizeLinesByConditionKeyword(part.content),
                type    = part.type,
                subtype = part.subtype,
                isComposite = True
            )
            newPartList.append(thisParsedPart)
        else:
            newPartList.append(part)
    return newPartList
...
'''
Everything right of an '=' becomes an EXPRESSION
'''
